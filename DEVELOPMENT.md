...
本指南面向维护 `v0.1.0-alpha` 的开发者。公开仓库名是 **iMarketMessage**，面向用户的 macOS 应用品牌是 **iMM**；Swift Package 保留 `MarketMessage` 兼容识别名，开发和发布优先使用 `iMM`，不代表额外的网络服务或账户。
...
`Config/com.marketmessage.monitor.plist.example` 只是 LaunchAgent 模板，包含占位路径，不会在构建或测试时安装服务。若用户确实需要后台检查，必须由用户复制模板、填写绝对路径、检查权限和日志位置，再由用户本人安装、启动、观察和卸载；贡献者不得在代码或 CI 中自动安装 LaunchAgent。
...
2. 只修改必要文件；不要提交 `.build`、`*.app`、密钥、证书、个人路径、真实 outbox、日志或截图。
...
5. 发布相关改动按 [`RELEASE_CHECKLIST.md`](RELEASE_CHECKLIST.md) 执行；签名和公证只由拥有相应 Apple 账户和私钥的用户完成，步骤见 [`docs/APPLE_SIGNING.md`](docs/APPLE_SIGNING.md)。
