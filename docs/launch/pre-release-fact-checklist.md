...
当前基础状态：仓库只有本地 outbox/dry-run foundation；尚无真实 sender、后台安装、签名 App 或 DMG。下列项目在实现和证据齐全前不得勾选，也不得在公开文案中写成“已提供”。
...
- [ ] 已用不含个人身份的测试数据验证重复事件、失败重试和重启后的行为，且测试消息只到明确配对的本人或不发送真实消息。

### 后台安装、签名 App 和 DMG

- [ ] 后台安装由用户明确发起，安装路径、运行用户、权限、状态检查、卸载和失败回滚均有文档；未写入用户名、绝对个人路径、token 或 key。
- [ ] 安装/卸载只影响本版本声明的文件，没有覆盖已有 gateway 或修改未授权的系统服务。
